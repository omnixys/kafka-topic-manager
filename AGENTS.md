<!-- repository: packages/ts/kafka-topic-manager | kind: PACKAGE | stack: ts-package -->

# kafka-topic-manager

> Repository development instructions for **kafka-topic-manager** (Shared Package).
> Stack: ts-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys Kafka topic manager for TypeScript.

- Repository path: `packages/ts/kafka-topic-manager` (relative to the Omnixys root)
...
Topic naming/schema conventions are contracts.
